...
export class ProdutoClient {
	...
	public async desativar(produto: Produto): Promise<void> {
		try {
			return (await this.axiosClient.put(`/desativar/${produto.id}`, produto)).data
		} catch (error:any) {
			return Promise.reject(error.response)
		}
	}

	public async findByFiltrosPaginado(pageRequest: PageRequest): Promise<PageResponse<Produto>> {
		try {
			let requestPath = ''
			...
			return (await this.axiosClient.get<PageResponse<Produto>>(requestPath, { params: { filtros: pageRequest.filter } })).data
		} catch (error:any) { 
			return Promise.reject(error.response) 
		}
	}
